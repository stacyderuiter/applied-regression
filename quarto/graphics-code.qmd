...
3. Plan and critique statistical graphics based on design principles.
...
after.  (Think:  "y depends on x".  Another way to remember the order is `y ~ x`: the y-axis label appears
...
or `binwidth` (the width of the bins).  Experiment with different bins, trying to find balance between too many and too few.  (If you don't provide `bins` or `binwidth` information, R will just make something up. You can usually do better if you take control.)
...
(in slightly different ways): `gf_density()` plots are filled-in, while `gf_dens()` just plots a line showing the shape of the distribution. 
