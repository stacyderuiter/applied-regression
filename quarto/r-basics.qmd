...
But you may have noticed that *you are not even using R.* At least, you haven't logged in to [RStudio server](https://r.cs.calvin.edu/auth-sign-in){target="_blank"}. (Ummm, wait - don't click that link! We don't need it...yet.)
...
(The `...` indicates that there may be some additional *input arguments* (input information we could provide to R) we could add
  eventually. Some functions need only one input, but if a function takes more than one argument, they are separated by commas. They have names, and if named (like: `function(input_name = value, input2_name = 'value')`) they can be in any order.
...
You can think of it as an *assignment arrow* -- it points *from* a value or item *toward a name* and assigns the name to the thing.
...
The code below - if corrected - *would* (on our server or in standalone R) run without an error and read in some baseball statistics from the file <https://sldr.netlify.app/data/baseball.csv>. Here in this tutorial, it will give the error: `! curl package not installed, falling back to using url()` -- there's not a straightforward fix, sorry,  but try it on the server if you want to prove to yourself that it works!

```{webr}
#| exercise: baseball-mistakes
read_csv(https://sldr.netlify.app/data/seball)
```
...
*Remember, `?` won't work on MI_lead because it's not a built-in R dataset. The dataset MI_lead is already read in for you, here...so you don't need to use `read_csv()`.*
